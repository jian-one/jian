 ...
 */
export function SidebarNavigation({ active, currentKind, profile, profiles, piAgents = profiles, sessions, localSessions, sidebarRef, onScroll, onAreaChange, onProfileChange, onSelectSession, onSelectLocal, onCreateLocal, onRemoveLocal, onOpenWorkspace, onRefresh, refreshingKind, onSettings, onDialog, onRelease, connectedSessionID, onDisconnect, visibleCount, onShowMore, username, onLogout, settingsOpen, onSettingsPage }: Props) {
  ...
  useEffect(() => { const refreshAgentEnabled = (event: Event) => { const next = (event as CustomEvent<Partial<typeof agentEnabled>>).detail; setAgentEnabled(next ? { codex: next.codex !== false, hermes: next.hermes !== false, pi: next.pi !== false } : { codex: localStorage.getItem('jian.codex-enabled') !== 'false', hermes: localStorage.getItem('jian.hermes-enabled') !== 'false', pi: localStorage.getItem('jian.pi-enabled') !== 'false' }); }; window.addEventListener('jian-agent-settings', refreshAgentEnabled); return () => window.removeEventListener('jian-agent-settings', refreshAgentEnabled); }, []);
  const [value, setValue] = useState<Area | ''>(areaFromActive);
  const [profileValue, setProfileValue] = useState(profile);
  const [codexWorkspaceValue, setCodexWorkspaceValue] = useState(''), [piWorkspaceValue, setPiWorkspaceValue] = useState('');
  ...
  const selectArea = (next: string) => {
    ...
    if (next === 'local' || next === 'codex' || next === 'hermes' || next === 'pi') onAreaChange(next);
  };
  ...
  const piDefaultRows = rowsFor(sessions, 'pi', 'default');
  const piWorkspaces = Array.from(new Set(piDefaultRows.map(session => displayWorkspacePath(session.workspace) || '未知工作区')));
  const codexRows = rowsFor(sessions, 'codex');
  const codexWorkspaces = Array.from(new Set(codexRows.map(session => displayWorkspacePath(session.workspace) || '未知工作区')));
    ...
    <NavigationMenu.Root className="navigation-menu" data-refreshing={refreshingKind || undefined} orientation="vertical" value={value} onValueChange={selectArea} delayDuration={0} skipDelayDuration={0}>
      <NavigationMenu.List className="navigation-menu-list">
        ...
        {agentEnabled.pi && <PrimaryNavigationItem value="pi" label="Pi" icon={<AgentIcon kind="pi" />} selected={currentKind === 'pi'} actions={<div className="navigation-menu-actions"><button className="icon" aria-label="Pi 设置" title="Pi 设置" onClick={() => onSettings('pi')}><Settings2 /></button><button className="icon" aria-label="刷新 Pi 会话" title="刷新 Pi 会话" disabled={!!refreshingKind} aria-busy={refreshingKind === 'pi'} onClick={() => onRefresh('pi')}><RefreshCw /></button></div>}>
          <NavigationMenu.Sub className="profile-navigation secondary-navigation" value={profileValue} onValueChange={selectProfile} orientation="vertical"><NavigationMenu.List className="profile-navigation-list secondary-navigation-list">
            {piRows.map(item => <NavigationMenu.Item key={item} value={item} className="profile-navigation-item secondary-navigation-item"><MenuTrigger variant="profile" onClick={event => { event.preventDefault(); toggleProfile(item); }}><span className="profile-navigation-label secondary-navigation-label"><small>ROLE</small><strong title={item}>{item}</strong></span></MenuTrigger><NavigationMenu.Content className="profile-navigation-content secondary-navigation-content" onEscapeKeyDown={preventEscapeDismiss} onFocusOutside={event => event.preventDefault()} onPointerDownOutside={event => event.preventDefault()}><section className="nav-sessions"><header><span>会话目录</span><button className="icon" aria-label={`在 ${item} 中新建 Pi 会话`} title="新建会话" onClick={() => onOpenWorkspace('pi', item)}><Plus /></button></header>{item === 'default' ? <NavigationMenu.Sub className="workspace-navigation secondary-navigation" value={piWorkspaceValue} onValueChange={setPiWorkspaceValue} orientation="vertical"><NavigationMenu.List className="workspace-navigation-list secondary-navigation-list">{piWorkspaces.map(workspace => <NavigationMenu.Item key={workspace} value={workspace} className="workspace-navigation-item secondary-navigation-item"><MenuTrigger variant="profile" onClick={event => { event.preventDefault(); setPiWorkspaceValue(value => value === workspace ? '' : workspace); }}><span className="secondary-navigation-label"><small>WORKSPACE</small><strong title={workspace}>{workspace}</strong></span></MenuTrigger><NavigationMenu.Content className="workspace-navigation-content secondary-navigation-content" onEscapeKeyDown={preventEscapeDismiss} onFocusOutside={event => event.preventDefault()} onPointerDownOutside={event => event.preventDefault()}><section className="nav-sessions nested-nav-sessions"><SessionList rows={piDefaultRows.filter(session => (displayWorkspacePath(session.workspace) || '未知工作区') === workspace)} listKind="pi" listProfile={`default:${workspace}`} activeID={active?.id} connectedID={connectedSessionID} visibleCount={visibleCount('pi', `default:${workspace}`)} onSelect={onSelectSession} onDialog={onDialog} onDisconnect={onDisconnect} onRelease={onRelease} onShowMore={() => onShowMore('pi', `default:${workspace}`)} /></section></NavigationMenu.Content></NavigationMenu.Item>)}</NavigationMenu.List></NavigationMenu.Sub> : <SessionList rows={rowsFor(sessions, 'pi', item)} listKind="pi" listProfile={item} activeID={active?.id} connectedID={connectedSessionID} visibleCount={visibleCount('pi', item)} onSelect={onSelectSession} onDialog={onDialog} onDisconnect={onDisconnect} onRelease={onRelease} onShowMore={() => onShowMore('pi', item)} />}</section></NavigationMenu.Content></NavigationMenu.Item>)}
          </NavigationMenu.List></NavigationMenu.Sub>
        </PrimaryNavigationItem>}
      </NavigationMenu.List>
    </NavigationMenu.Root>
