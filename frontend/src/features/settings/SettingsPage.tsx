...
type Props = { theme: Theme; onThemeChange: (theme: Theme) => void; terminalTheme: TerminalTheme; onTerminalThemeChange: (theme: TerminalTheme) => void; onAgentEnabledChange?: (kind: 'codex' | 'hermes', enabled: boolean) => Promise<void> };
...
function LaunchArguments({ kind, settings, setSettings }: { kind: AgentKind; settings: AgentSettings; setSettings: (update: (value: AgentSettings) => AgentSettings) => void }) {
  ...
  const key = kind === 'codex' ? 'codex_args' : 'hermes_args';
...
function AgentRosterItem({ kind, settings, username, availableProfiles, save, setSettings }: { kind: RosterKind; settings: AgentSettings; username: string; availableProfiles: string[]; save: (kind: RosterKind, enabled: boolean) => Promise<void>; setSettings: (update: (value: AgentSettings) => AgentSettings) => void }) {
  ...
  const label = kind === 'local' ? 'Local' : kind === 'codex' ? 'Codex' : 'Hermes';
  const [profilePicking, setProfilePicking] = useState(false);
  const toggle = (next: boolean) => void save(kind, next);
  const field = (key: 'codex_bin' | 'path' | 'hermes_home' | 'hermes_bin', fieldLabel: string, hint: string) => <label className="setting-field"><span>{fieldLabel}<small>{hint}</small></span><input value={settings[key] || ''} onChange={event => setSettings(value => ({ ...value, [key]: event.target.value }))} /></label>;
  ...
  const environmentVariables = kind !== 'local' && <EnvironmentVariables values={kind === 'codex' ? settings.codex_env || [] : settings.hermes_env || []} onChange={values => setSettings(value => ({ ...value, [kind === 'codex' ? 'codex_env' : 'hermes_env']: values }))} />;
  ...
  return <Accordion.Item className="agent-roster-item" value={kind}><div className="agent-toggle-row"><Accordion.Header><Accordion.Trigger className="agent-roster-trigger" aria-label={`${label}设置，展开或收起详细配置`}><span className="agent-toggle-copy"><AgentIcon kind={kind === 'local' ? 'local' : kind} /><strong>{label}</strong></span><ChevronDown className="agent-roster-chevron" aria-hidden="true" /></Accordion.Trigger></Accordion.Header><Switch.Root className="agent-switch" checked={enabled} disabled={kind === 'local'} onCheckedChange={kind === 'local' ? undefined : toggle} aria-label={kind === 'local' ? 'Local 已激活' : `启用 ${label}`}><Switch.Thumb className="agent-switch-thumb" /></Switch.Root></div><Accordion.Content className="agent-roster-content"><div className="agent-roster-fields">{kind === 'local' ? <><div className="setting-field"><span>系统用户名</span><p className="setting-readonly">{username || '未知用户'}</p></div><div className="setting-field"><span>启动时的 PATH</span><p className="setting-readonly setting-path">{settings.path || '未设置'}</p></div>{localProfiles}</> : kind === 'codex' ? <>{field('codex_bin', 'CODEX_BIN', 'Codex 可执行文件路径')}{environmentVariables}<LaunchArguments kind="codex" settings={settings} setSettings={setSettings} /></> : <>{field('hermes_home', 'HERMES_HOME', 'Hermes 配置与 profile 根目录')}{field('hermes_bin', 'HERMES_BIN', 'Hermes 可执行文件路径')}{environmentVariables}<LaunchArguments kind="hermes" settings={settings} setSettings={setSettings} /><fieldset className="profile-settings"><legend>显示会话的 profiles</legend><small>只显示勾选 profile 的 Hermes 会话列表。</small>{availableProfiles.map(profile => <label key={profile}><input type="checkbox" checked={settings.hermes_profiles.includes(profile)} onChange={() => toggleProfile(profile)} />{profile}</label>)}</fieldset></>}</div><footer><button onClick={() => void save(kind, enabled)}>保存 {label} 设置</button></footer></Accordion.Content></Accordion.Item>;
}
...
function TerminalSettings() {
  ...
  const release = async () => { if (!confirm) return; setBusy(true); try { const path = confirm.type === 'all' ? '/settings/terminals/release-all' : `/settings/terminals/${encodeURIComponent(confirm.terminal!.id)}/release`; await api(path, { method: 'POST' }); if (confirm.type === 'all') window.dispatchEvent(new Event('jian-release-all-terminals')); setConfirm(null); await load(); } catch (e) { setError(errorMessage(e)); } finally { setBusy(false); } };
  return <div className="terminal-settings-content"><header className="terminal-settings-heading"><div className="settings-section-intro"><span className="eyebrow">TERMINAL MANAGER</span><p>当前活跃的 PTY 会话，可手动释放。</p></div></header>{error && <p className="error" role="alert">{error}</p>}{status && <TerminalPool terminals={status.active_pool} onEnter={enter} onRelease={terminal => setConfirm({ type: 'release', terminal })} onReleaseAll={() => setConfirm({ type: 'all' })} />}{!status && !error && <p className="muted">正在读取 TerminalManager 状态…</p>}<ConfirmDialog open={!!confirm} title={confirm?.type === 'all' ? '释放所有会话？' : '释放这个会话？'} description={confirm?.type === 'all' ? '所有 terminal 子进程都会被强制停止，当前浏览器连接也会断开。' : `“${confirm?.terminal?.title || confirm?.terminal?.id}”将被强制停止，正在执行的任务会中断。`} confirmLabel="确认释放" danger busy={busy} onConfirm={() => void release()} onClose={() => setConfirm(null)} /></div>;
}
...
export function SettingsPage({ theme, onThemeChange, terminalTheme, onTerminalThemeChange, onAgentEnabledChange: configuredAgentChange }: Props) {
  ...
  const onAgentEnabledChange = configuredAgentChange || (async (kind: 'codex' | 'hermes', enabled: boolean) => { localStorage.setItem(`jian.${kind}-enabled`, String(enabled)); });
